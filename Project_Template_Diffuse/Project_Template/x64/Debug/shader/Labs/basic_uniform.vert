...
{ 
  //ambient
  ambient = Material.ambient * Light.Ld;
  ...
  vec3 v = normalize(vec3(viewPos) - vec3(pos));

  float rDotV = pow(max( dot(r,vec3(viewPos)), 0.0), 0. );

  vec3 specular = Material.Ks * Light.Ld * rDotV;

  ReflectionIntensity = specular;
